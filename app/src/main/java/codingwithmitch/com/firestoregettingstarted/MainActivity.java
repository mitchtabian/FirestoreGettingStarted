        ...
        IMainActivity
{
    ...
    private void getNotes(DocumentSnapshot lastVisible){
        ...
        notesQuery.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if(task.isSuccessful()){
                    ...
                    if(mNotes.size() > 0){
                        if(mNotes.get(mNotes.size() - 1).getTitle().equals("")){
    ...
    private void initRecyclerView(){
        if(mNoteRecyclerViewAdapter == null){
            mNoteRecyclerViewAdapter = new NoteRecyclerViewAdapter(mNotes);
        }
        ...
        mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (!recyclerView.canScrollVertically(1)) {
                    Log.d(TAG, "onScrolled: List has reached the bottom. Getting more notes.");
                    Note note = new Note();
                    note.setTitle("");
                    mNotes.add(note);
                    mNoteRecyclerViewAdapter.notifyDataSetChanged();
                    getNotes(mLastVisible);
                }
            }
        });
